...
import { GOOGLE_CLIENT_ID, api } from '../../lib/auth'
 ...
 * Google sign-in.
 *
 * With VITE_GOOGLE_CLIENT_ID set, this renders the real Google Identity Services
...
export default function GoogleButton({ onSession, onError, label = 'Continue with Google' }) {
  ...
  if (GOOGLE_CLIENT_ID && !scriptFailed) return <div ref={host} className="flex justify-center" />
  ...
  return (
    ...
    >
      ...
      {busy ? 'Signing in…' : GOOGLE_CLIENT_ID ? label : `${label} (demo)`}
    </button>
  )
}
